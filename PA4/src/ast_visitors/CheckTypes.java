...
//    public Type toType(IType t) {
//    		if (t instanceof IntType) return Type.INT;
//    		if (t instanceof ByteType) return Type.BYTE;
//    		if (t instanceof ColorType) return Type.COLOR;
//    		return null;
//    }
   ...
   */
		...
		// Verify parameters fit the formal params
 		...
 		for (VarSTE param: ((MethodScope)this.mCurrentST.mScopeStack.peek()).getFormals()) {
			...
			if (arg == null) {
				...
				"Method call " + node.getId() + " has too few arguments",
			...
			if (this.mCurrentST.getExpType(arg) != param.type) {
				...
				if (param.type == Type.INT && this.mCurrentST.getExpType(arg) == Type.BYTE) break;
				throw new SemanticException(
			"Method arg " + param.name + " of incorrect type. Should be " + param.type + ", is " + this.mCurrentST.getExpType(arg),
				arg.getLine(), arg.getPos());
			}
	...
	public void outIdLiteral(IdLiteral node) {
		if (mCurrentST.lookupSymbol(node.getLexeme()) == null) {
    		throw new SemanticException(
			"Undefined symbol " + node.getLexeme(),
   ...
   public void outNotExp(NotExp node) {
	...
	public void outMeggyToneStart(MeggyToneStart node) {
		if (this.mCurrentST.getExpType(node.getToneExp()) != Type.INT) {
			throw new SemanticException(
			"Tone: Invalid argument type for method MeggyToneStart",
			node.getToneExp().getLine(),
			node.getToneExp().getPos());
		}
		if (this.mCurrentST.getExpType(node.getDurationExp()) != Type.INT) {
			throw new SemanticException(
			"Duration: Invalid argument type for method MeggyToneStart",
